...
def format_walk_forward(wf: WalkForwardResult, title: str = "Walk-forward") -> str:
    ...
    out.append("")
    out.append("  Manual tradeability")
    out.append(f"  {'  Trades per session':<28} {_fmt(d.get('trades_per_day')):>12}")
    out.append(f"  {'  Avg hold (minutes)':<28} {_fmt(d.get('avg_hold_min'), '{:.0f}'):>12}")
    out.append(f"  {'  Longest hold (minutes)':<28} {_fmt(d.get('max_hold_min'), '{:.0f}'):>12}")
    out.append(f"  {'  Busiest session (trades)':<28} {_fmt(d.get('busiest_day'), '{:.0f}'):>12}")
    out.append("")
